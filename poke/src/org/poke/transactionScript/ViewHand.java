		...
		if(res != null) { // user has already logged in
			try {
				...
				if(gameRdg.getChallengeeId() != userId && gameRdg.getChallengerId() != userId) {
					...
					List<HandsRDG> handsList = HandsRDG.find(userId);
					request.setAttribute("hands", handsList);
					request.getRequestDispatcher("WEB-INF/listHands.jsp").forward(request, response);
				}
			...
			request.setAttribute("status", "fail");
			request.setAttribute("message", "Whoops, something goes wrong! You can not view hand without log in first.");
			request.getRequestDispatcher("WEB-INF/failure.jsp").forward(request, response);
		}
	}
