		...
		if(res != null) { // user has already logged in
			try {
				...
				if(gameRdg.getChallengeeId() != userId && gameRdg.getChallengerId() != userId) {
					...
					request.setAttribute("message", "Can not retire since this game has nothing to do with you.");
					request.getRequestDispatcher("WEB-INF/failure.jsp").forward(request, response);
				} else {
					gameRdg.delete();
					request.setAttribute("status", "success");
					request.setAttribute("message", "The game has just been deleted, you can retire now.");
			...
			request.setAttribute("status", "fail");
			request.setAttribute("message", "Whoops, something goes wrong! You can not retire without log in first.");
			request.getRequestDispatcher("WEB-INF/failure.jsp").forward(request, response);
		}
